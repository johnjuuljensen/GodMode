...
- **Before Allow**, a client fetches `GetPermissionDetail`: the whole command for `Bash` and `PowerShell`, the path and the whole new text for `Edit`, `MultiEdit`, `Write` and `NotebookEdit`, and the input as indented JSON for any other tool, cut at 16384 characters with `DetailTruncated` set. The call runs with all of it.
