  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      const config: McpServerConfig = connection?.type === 'http'
        ? { url: connection.url, env: Object.keys(env).length > 0 ? env : undefined }
        : {
            command: 'npx',
              ...
              >
                ...
                <div className="mcp-server-info">
                  <div className="mcp-server-name">
                    {server.displayName}
                    {server.isVerified && <span className="mcp-verified-badge" title="Verified">&#10003;</span>}
                  </div>
                  {server.description && (
                ...
                <div className="mcp-server-meta">
                  {server.useCount != null && server.useCount > 0 && `${server.useCount.toLocaleString()} uses`}
                </div>
              </div>
            ))}
          </div>
