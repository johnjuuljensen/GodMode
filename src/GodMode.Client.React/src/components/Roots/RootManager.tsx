...
import type {
  RootTemplate, RootPreview, RootGenerationRequest, SchemaFieldDefinition,
          ...
          {view === 'list' && hub && (
            <>
              <RootList
                ...
                onRefresh={() => refreshProjects(selectedServerIndex)}
              />
...
function RootList({ roots, hub, serverIndex, onRefresh }: {
...
function CreateRootPane({ hub, serverIndex, profiles, onCreated, onFooterAction }: {
  hub: InstanceType<typeof import('../../signalr/hub').GodModeHub>;
  serverIndex: number;
...
function ImportRootPane({ hub, serverIndex, onInstalled }: {
  hub: InstanceType<typeof import('../../signalr/hub').GodModeHub>;
  serverIndex: number;
  onInstalled: () => void;
}) {
