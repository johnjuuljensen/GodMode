...
The server injects `GODMODE_PROJECT_ID`, `GODMODE_PROJECT_TOKEN` (per-project token that authorizes only `/api/internal/*` for that project) and `GODMODE_SERVER_URL` (`http://localhost:{port}`, which is why a server bound to another address keeps a loopback binding too). It finds the bridge through the `McpBridgePath` setting (or `GODMODE_MCP_BRIDGE_PATH`), or next to its binaries, and refuses to start without it.
